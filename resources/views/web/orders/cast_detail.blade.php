    ...
    <div class="slider cast-photo__show">
      @if($cast['avatars'])
        @foreach ($cast['avatars'] as $avatar)
          ...
          <img data-lazy="{{ $avatar['path'] }}" alt="">
          @else
          <img data-lazy="{{ asset('assets/web/images/gm1/ic_default_avatar@3x.png') }}" alt="">
          @endif
        @endforeach
      @else
        <img class="image-default" data-lazy="{{ asset('assets/web/images/gm1/ic_default_avatar@3x.png') }}" alt="">
      @endif
    </div>
...
<div class="cast-call-btn" id="cast-order-call">
  <div class="btn-m"><a href="{{ route('guest.orders.get_step3') }}" >このキャストを指名する</a></div>
</div>
<input type="hidden" value="{{ $cast['id'] }}" id="cast-id-info">
@endsection
@section('web.script')
