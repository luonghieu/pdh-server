                        ...
                        <div class="info-table col-lg-10">
                            <form action="">
                                    ...
                                    <tr>
                                        ...
                                        <td class="edit-order-detail wrap-status" id="order-status">
                                            ...
                                            @if (App\Enums\OrderPaymentStatus::EDIT_REQUESTING == $order->payment_status)
                                                <button class="change-time payment-request btn-order-call"
                                                        data-toggle="modal" data-target="#payment-request">
                                                    ステータスを売上申請待ちに切り替える
                                                </button>
                                                <button class="change-time btn-pay-point" data-toggle="modal"
                                                        data-target="#pay-point">ステータスをポイント決済完了に切り替える
                                                </button>
                                            @endif
                                        </td>
                                    </tr>
                                ...
                                <div class="panel-body">
                                    ...
                                    <div class="display-title change-cast-order-call">
                                        <a href="" data-toggle="modal" data-target="#choose-cast-candidate"
                                           id="popup-cast-candidate">+別のキャストを追加する</a>
                                    </div>
                                </div>
                                ...
                                <div class="panel-body">
                                    ...
                                    <div class="display-title change-cast-order-call">
                                        <a href="" data-toggle="modal" data-target="#choose-cast-matching"
                                ...
                                <div class="modal-dialog" role="document">
                                    <div class="modal-content">
                                            ...
                                            <div class="panel-body handling">
                                                <div class="search">
                                                    <input type="text" class="form-control input-search"
                                                           placeholder="ユーザーID,名前" name="search" value="" id="search">
                                                </div>
                                            </div>
                                            <div class="wrapper-table">
                                                <div class="cast-nominee"></div>
                                                <input type="hidden" id="cast_ids" name="cast_ids"
                                                       value="{{ old('cast_ids') }}"/>
                                                <table class="table table-striped table-bordered bootstrap-datatable"
                                                       id="nomination-table">
                                                    <thead>
                                        ...
                                        <div class="modal-footer">
                                            ...
                                            <button type="submit" class="btn btn-accept" id="add-cast-nominee">
                                                このキャストを選択する
                                            </button>
                                        </div>
                                    </div>
                            ...
                            <div class="modal fade" id="choose-cast-candidate" tabindex="-1" role="dialog"
                                 aria-labelledby="exampleModalLabel" aria-hidden="true">
                                <div class="modal-dialog" role="document">
                                    <div class="modal-content">
                                        <div class="modal-body">
                                        ...
                                        <div class="modal-footer">
                                            ...
                                            <button type="submit" class="btn btn-accept" id="add-cast-candidate">
                                                このキャストを選択する
                                            </button>
                                        </div>
                                    </div>
                            ...
                            <div class="modal fade" id="choose-cast-matching" tabindex="-1" role="dialog"
                                 aria-labelledby="exampleModalLabel" aria-hidden="true">
                                <div class="modal-dialog" role="document">
                                    <div class="modal-content">
                                        <div class="modal-body">
                                            ...
                                            <div class="panel-body handling">
                                                <div class="search">
                                                    <input type="text" class="form-control input-search"
                                                           placeholder="ユーザーID,名前" name="search" value="">
                                                </div>
                                            </div>
                                            <div class="wrapper-table">
                                                <table class="table table-striped table-bordered bootstrap-datatable"
...
@section('admin.js')
    <script type="text/javascript">
        ...
        let selectedNomination = JSON.parse(JSON.parse(JSON.stringify('<?php echo json_encode($castsNominee, true) ?>')));
        let selectedCandidate = JSON.parse(JSON.parse(JSON.stringify('<?php echo json_encode($castsCandidates, true)?>')));
        let selectedMatching = JSON.parse(JSON.parse(JSON.stringify('<?php echo json_encode($castsMatching, true) ?>')));
        const baseCastClass = '<?php echo $order->class_id ?>';
        const orderId = '<?php echo $order->id ?>';
        const baseCastsMatched = JSON.parse(JSON.parse(JSON.stringify('<?php echo json_encode($castsMatching, true) ?>')));
        const baseCastsNominee = JSON.parse(JSON.parse(JSON.stringify('<?php echo json_encode($castsNominee, true) ?>')));
        const baseCastsCandidate = JSON.parse(JSON.parse(JSON.stringify('<?php echo json_encode($castsCandidates, true)?>')));
        const castClasses = JSON.parse(JSON.parse(JSON.stringify('<?php echo json_encode($castClasses, true) ?>')));
        const orderType = '<?php echo $order->type ?>';
        const orderTypeDesc = JSON.parse(JSON.parse(JSON.stringify('<?php echo json_encode($orderTypeDesc, true) ?>')));
        const orderStatusDesc = JSON.parse(JSON.parse(JSON.stringify('<?php echo json_encode($orderStatusDesc, true) ?>')));
