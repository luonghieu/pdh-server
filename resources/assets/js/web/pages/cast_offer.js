...
function loadCouponsCastOrder()
{
  var couponId = null;

  if(localStorage.getItem("cast_offer")) {
    var orderNominate = JSON.parse(localStorage.getItem("cast_offer"));
      ...
      if(couponId) {
        ...
        window.axios.post('/api/v1/orders/price',params)
          .then(function(response) {
            ...
            helper.updateLocalStorageValue('cast_offer', params);
  ...
  $('body').on('change', "#coupon-order-nominate", function(){
    ...
    if(parseInt(couponId)) {
      ...
      if(couponIds.indexOf(parseInt(couponId)) > -1) {
        ...
        helper.updateLocalStorageValue('cast_offer', paramCoupon);
      ...
      if(localStorage.getItem("cast_offer")){
        var orderParams = JSON.parse(localStorage.getItem("cast_offer"));
        if(orderParams.coupon) {
          helper.deleteLocalStorageValue('cast_offer','coupon');
        }
      }
    }
...
function handlerSelectedTransfer()
{
  ...
  transfer.on("change",function(){
    ...
    helper.updateLocalStorageValue('cast_offer', param);
...
$(document).ready(function(){
  ...
  $('body').on('change', ".input-time-join",function(){
    ...
    helper.updateLocalStorageValue('cast_offer', updateTime);
  ...
  selectedPrefectureNomination.on("change",function(){
    ...
    helper.deleteLocalStorageValue('cast_offer','select_area');
    helper.deleteLocalStorageValue('cast_offer','text_area');
