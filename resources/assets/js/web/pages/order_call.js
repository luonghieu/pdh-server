$(document).ready(function(){
    ...
    if($("#ge2-1-x input:radio[name='cast_class']:checked").length){
        ...
        if (castClass == 3) {
            $('.notify-campaign-over-cast-class span').text('â€»â€ãƒ€ã‚¤ãƒ¤ãƒ¢ãƒ³ãƒ‰â€ã¯ã‚­ãƒ£ãƒ³ãƒšãƒ¼ãƒ³å¯¾è±¡å¤–ã§ã™');
            $('.notify-campaign-over-cast-class').css('display','block');
        }

        if (castClass == 2) {
            $('.notify-campaign-over-cast-class span').text('â€»â€ãƒ—ãƒ©ãƒãƒŠâ€ã¯ã‚­ãƒ£ãƒ³ãƒšãƒ¼ãƒ³å¯¾è±¡å¤–ã§ã™');
            $('.notify-campaign-over-cast-class').css('display','block');
        }
    ...
    $(".date-select__ok").on("click",function(){
        ...
        if((!area || (area=='ãã®ä»–' && !otherArea)) || !castClass ||
    ...
    $('#list-cast-order').on("change", ".cast_block .select-casts", function(event){
        ...
        if($('.select-casts:checked').length > countIds) {
            var text = ' æŒ‡åã§ãã‚‹ã‚­ãƒ£ã‚¹ãƒˆã¯'+ countIds + 'åã§ã™';
            ...
            if ($(this).is(':checked')) {
                if(localStorage.getItem("order_call")){
                    ...
                    if (arrIds) {
                        if(arrIds.length < countIds) {
                            ...
                            $('.label-select-casts[for='+  id  +']').text('ãƒªã‚¯ã‚¨ã‚¹ãƒˆä¸­');
                        } else {
                            var text = ' æŒ‡åã§ãã‚‹ã‚­ãƒ£ã‚¹ãƒˆã¯'+ countIds + 'åã§ã™';
                            $('#content-message h2').text(text);
                            $('#max-cast').prop('checked', true);
                        ...
                        if(arrIds.length) {
                            $('#sb-select-casts a').text('æ¬¡ã«é€²ã‚€(3/4)');
                        } else {
                            $('#sb-select-casts a').text('å¸Œæœ›ãƒªã‚¯ã‚¨ã‚¹ãƒˆã›ãšã«é€²ã‚€(3/4)');
                        }
                ...
                $('.label-select-casts[for='+  id  +']').text('ãƒªã‚¯ã‚¨ã‚¹ãƒˆã™ã‚‹');
            }
        }
    ...
    if((area || (area=='ãã®ä»–' && otherArea)) && time && castClass && duration) {
        $("button[type='submit'][name='sb_create']").removeClass('disable');
        $("button[type='submit'][name='sb_create']").prop('disabled', false);
    }
    ...
    buttonGreen.on("change",function(){
        ...
        if((!area || (area=='ãã®ä»–' && !otherArea)) || !time || !castClass ||
    ...
    timeButton.on("change",function(){
        ...
        if( !time || (!area || (area=='ãã®ä»–' && !otherArea)) ||
    ...
    castClass.on("change",function(){
        ...
        if( !time || (!area || (area=='ãã®ä»–' && !otherArea)) || !castClass ||
            (!duration || (duration<1 && 'other_duration' != duration)) ||(!totalCast || totalCast<1) || (time=='other_time' && !date)) {
            $("button[type='submit'][name='sb_create']").addClass("disable");
            $("button[type='submit'][name='sb_create']").prop('disabled', true);
        } else {
    ...
    if (checkNumber > 2) {
        if (checkNumber == 3) {
            $('.notify-campaign-over span').text('â€»3åã¯ã‚­ãƒ£ãƒ³ãƒšãƒ¼ãƒ³å¯¾è±¡å¤–ã§ã™');
        }

        if (checkNumber == 4) {
            $('.notify-campaign-over span').text('â€»4åã¯ã‚­ãƒ£ãƒ³ãƒšãƒ¼ãƒ³å¯¾è±¡å¤–ã§ã™');
        }
